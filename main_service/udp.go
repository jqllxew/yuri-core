...
func StartHolePunchServer(port string, server *net.UDPConn) {
	...
	for {
		...
		if err != nil {
			verbose.DebugInfo(2, "UDP read error from", ClientAddr.String())
			continue
		}
...
func RecvHolePunchMessage(data []byte, len int, UDPClient *net.UDPAddr, server *net.UDPConn) {
	...
	if !p.PraseUDPpacket(data, len) {
		verbose.DebugInfo(2, "UDP had a illegal packet from", UDPClient.String())
		return
	}
	if p.IsHeartbeat() {
		return
	...
	if err != nil {
		verbose.DebugInfo(2, "Error : Prasing externalIpAddress error !")
		return
	}
	//找到对应玩家
